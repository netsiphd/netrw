...
def visualize_example_full_rewire(RewiringTechnique=LocalEdgeRewiring,
    ...
    if list_of_graphs == []:
        list_of_graphs = [nx.karate_club_graph(), 
    ...
    fig, ax = plt.subplots(len(list_of_graphs),2,
                           figsize=(base_width*2,
                                    base_height*len(list_of_graphs)),
                           dpi=100)

    ax[(0,0)].text(1.1, 1.2, "Method: "+rewiring_technique_label,
    ...
    for ix,G0 in enumerate(list_of_graphs):
        ...
        nx.draw_networkx_nodes(G0, pos, ax=ax[(ix,0)], node_size=ns,
                               node_color='w', edgecolors=n_ec, linewidths=lw)
        nx.draw_networkx_edges(G0, pos, ax=ax[(ix,0)], edge_color='.5',
                               width=ew, alpha=0.35)

        # draw rewired network
        nx.draw_networkx_nodes(G, pos, ax=ax[(ix,1)], node_size=ns,
                               node_color='w', edgecolors=n_ec, linewidths=lw)
        nx.draw_networkx_edges(G, pos, ax=ax[(ix,1)], edge_color='.5',
                               width=ew, alpha=0.35)
